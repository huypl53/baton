...
- Canonical helper (required for command execution): `./scripts/tmux-send-and-enter.sh -t <target> -- '<text>'`
  - Always sends literal text, then Enter (`C-m`) to submit.
  - Accepts both `session:window.pane` and `%<pane_id>` targets.
- Manual fallback pattern (only when helper is unavailable):
  - `tmux -S "$SOCKET" send-keys -t target -l -- "$cmd"`
  - `tmux -S "$SOCKET" send-keys -t target C-m`
- When composing inline commands, use single quotes or ANSI C quoting to avoid expansion: `tmux ... send-keys -t target -- $'python3 -m http.server 8000'` (then send `C-m`).
- To send control keys: `tmux ... send-keys -t target C-c`, `C-d`, `C-z`, `Escape`, `C-m`, etc.
...
`./scripts/wait-for-text.sh` polls a pane for either text matches or idle-state confirmation. Works on Linux/macOS with bash + tmux + grep + sha256sum.

```bash
./scripts/wait-for-text.sh -t session:0.0 -p 'pattern' [-F] [-T 20] [-i 1] [-l 2000]
./scripts/wait-for-text.sh -t %12 --idle [-T 30] [--idle-confirm-seconds 10] [-i 1] [-l 2000]
```

- `-t`/`--target` pane target (required; supports `session:window.pane` and `%<pane_id>`)
- `-p`/`--pattern` regex to match (required unless `--idle`)
- `-F`/`--fixed` treat pattern as fixed string
- `--idle` enable hash-based idle detection mode
- `--idle-confirm-seconds` unchanged-duration threshold to confirm idle (default 10)
- `-T` timeout seconds (integer, default 15)
- `-i` poll interval seconds (default 1)
- `-l` history lines to search/hash from pane (integer, default 1000)
- Exits 0 on first match (pattern mode) or confirmed idle (idle mode), 1 on timeout. On failure prints last captured text to stderr.
